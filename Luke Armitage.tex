...
The following notation applies throughout: $\mathbb{K}$ denotes a field. $\R^+$ 
denotes the non-negative reals. $\Real z$ refers to the real part of the complex 
number $z$. An ideal of an algebra is, unless otherwise stated, taken to mean a 
two-sided ideal. An algebra $A$ is \emph{commutative} or \emph{Abelian} if
\begin{align*}
...
\todo[inline]{define hilbert space here, introduce $\BH$, continuity and boundedness}
...
$\H$ and by \eqref{eqn:sum2}, $x^n$ converges to $(x_i)$ as $n$ tends to 
...
I\right\}$, denoted $\oplus T_i$. For $S_i,T_i$ in $\B(\H_i)$, $\alpha,\beta$ in 
$\C$, we have 
\begin{align*}
...
intersections and arbitrary unions, and contains both $X$ and the empty set. A 
\emph{neighbourhood} of a point $x$ is a subset of $X$ which contains an open 
...
\begin{itemize}
	\item \emph{Hausdorff} if, for every distinct pair of points $x,y$ of $X$, there 
	is a neighbourhood of $x$ which is disjoint from some neighbourhood of $y$;
...
A \emph{topological vector space} is a vector space $V$ over $\mathbb K$, together with a topology on $V$ such that the vector space operations $V\times V\to V:(x,y)\mapsto x+y$ and $\mathbb{K}\times V\to V:(\lambda,x)\mapsto \lambda x$ are continuous. Recall that an \emph{extreme point} of a convex subset $X_0$ of a topological vector space is a point $x$ for which an expression
\begin{align*}
...
\begin{proof}
	...
	Given $f$ and $g$ in $\CX$, we have \todo{fix inequality}
	\begin{align*}
				\left\|fg\right\| 
		&= 		\sup_{x\in X}|(fg)(x)|												\\
		&= 		\sup_{x\in X}|f(x)g(x)|												\\
...
We will just state the necessary definitions and results here, and give sources 
for proofs.
...
\begin{thm*}[Continuous functional calculus, {\cite[4.1.3]{kadison83}}]
	...
	unique continuous mapping $C(\spec a):f\mapsto f(a)$ such that:
	\begin{enumerate}
...
\begin{prop}[{\cite[4.2.3(i)]{kadison83}}]\label{prop:423}
	Given $f$ in $C(\spec a)$, $f(a)$ is positive \todo{not yet defined} if and only 
	if $f(t)\geq0$ for all $t$ in $\spec a$.
\end{prop}

\begin{thm*}[{\cite[4.1.5]{kadison83}}]
	If $B$ is a $C^\ast$-subalgebra of a $C^\ast$-algebra $A$, and $b$ is an element 
	of $B$, then 
	\begin{align*}
		\sigma_B(b) =\sigma_A(b).
	\end{align*}
...
\begin{defn}
	...
	self-adjoint and $\spec a\subseteq \R^+$. Denote the set of positive elements in 
	$A$ by $A^+$. Then $A_{sa}$ is a partially ordered vector space with $\leq$ 
	given by
	\begin{align*}
...
\begin{prop}[{\cite[4.3.2]{kadison83}}]\label{prop:432}
	...
	$\rho$ is bounded and $\left\|\rho\right\| = \rho(\1)$.
\end{prop}
\begin{proof}
...
\begin{lemma}[{\cite[3.2.20]{kadison83}}]\label{lemma:3220}
	The maximal ideal space of an Abelian Banach algebra $A$ form a compact subset of 
...
\emph{operator algebras}. Here we will demonstrate that $\B(\H)$ is a 
...
explaining a good deal of the terminology used here. 
...
\textquote[{\cite[p.~211]{vonneumann32}}]{In the case of noncommuting 
...
\todo{give chapter reference not page.}
...
$\CX$. So, what happens if we apply the GNS construction to $\rho_x$?
...
\bibitem{dixmier77}
	Dixmier, J., 1977. \emph{C*-algebras}. 
