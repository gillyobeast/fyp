...
\chapter{Starter mathematics. Mathematical Pikachu, if you will.}

\todo[inline]{define here: Banach/Hilbert space. algebra/subalgebra/ideal/homomorphism. linear functional on a linear space. topological space/hausdorff/compact/locally compact/extreme point. partially ordered vector space?}
...
\] 
(note that this is automatically true for finite $I$).
For $x=(x_i)$ in $\oplus\H_i$, define an element $Tx$ in $\oplus\H_i$ by 
$Tx=(T_ix_i)$. Then $T:\H\to\H:x\mapsto Tx$ is a bounded linear operator, called the \emph{direct
sum} of the collection $\{T_i\in\B{(\H_i)} ~|~ i \in I\}$, denoted $\oplus T_i$. For $S_i,T_i$
in $\B(\H_i)$, $\alpha,\beta$ in $\C$, we have 
\begin{align*}
...
\begin{enumerate}
	...
	\item	$\alpha a\in A^+$ for all $\alpha\in \R^+$,
	\item	$a+b \in A^+$.
\end{enumerate}
The unit $\1$ is positive, and for any $a\in A_{sa}$ we have $-\|a\|\1 \leq a \leq \|a\|\1$.
\todo{justify.} With commuting elements $a,b\in A_{sa}$, we have $(ab)^\ast = b^\ast a^\ast = ba =
...
\begin{defn}
	...
	$a\in A^+$. A \emph{state} on $A$ is a positive linear functional $\rho$ such that $\|\rho\|=1$ and
...
functional $\rho$ is a pure state on $A$ if and only if the restriction $\rho|_{A_{sa}}$ is a pure
 state on $A_{sa}$.
Every pure state on $A_{sa}$ extends to a pure state on $A$. \todo{justify (4.3.13, Hahn-Banach.)}
...
\begin{proof}[Proof. (Adapted from 3.4.6)]
	...
	we get $0\leq\tau(\1)\leq\rho(\1) = 1$. Let $\lambda = \tau(\1)$. If $\lambda=0$, then for any 
	$a\in A_{sa}$, applying $\tau$ to $-\|a\|\1\leq a\leq\|a\|\1$ gives
	\begin{align*}
