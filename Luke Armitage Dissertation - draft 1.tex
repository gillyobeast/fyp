...
\begin{lemma}[{\cite[4.3.4]{kadison83}}]
...
\begin{prop}[{\cite[3.4.6]{kadison83}}]\label{pure1}
...
\begin{proof}[Proof.(Adapted from K\&R, 4.4.1)]
	...
	$\rho_b|_{A_{sa}} \leq \rho|_{A_{sa}}$, and it follows from Proposition \ref{pure1} that 
...
\begin{proof}
	...
	construction. We show that $\phi$ is a faithful representation.
	
	Now for any $a\not=0$ in $A$, there \todo{show}%%
	is a pure state on $A$ such that $\rho(a)\not=0$. By the GNS
	construction, $\rho(a)= \langle \pi_\rho (a) x_\rho, x_\rho \rangle$, so $\pi_\rho\not=0$.
