...
\section{History of $C^\ast$-Algebras}
...
\chapter{Starter mathematics}

\todo[inline]{notation: $\R^+$ non neg reals. define here: Banach/Hilbert space. algebra/subalgebra/ideal/homomorphism. linear functional on a linear space. topological space/hausdorff/compact/locally compact/extreme point. partially ordered vector space?}

Recall that an \emph{extreme} point $x$ of a convex subset $X$ of a topological space is one for 
which an expression
\begin{align*}
	x = \alpha x_1 +(1-\alpha)x_2,
\end{align*}
for $0\leq\alpha\leq1$ and $x_1,x_2\in X$, implies that $x_1=x=x_2$ -- for example, the vertices of a 
polygon embedded in $\R^2$ are extreme points of that polygon.

\section{Normed linear spaces}
\begin{thm}[Hahn-Banach Extension theorem] \label{hahnbanach}
...
\section{Constructions on Hilbert spaces}\label{hscons}
...
\[
	\H:= \{x=(x_i) ~|~ x_i \in \H_i \mbox{ for each }i 
...
\begin{align*}
	...
	&< \infty.
\end{align*}
Hence, the sequence $(x_i+y_i)$ is in $\H$. Thus we can define addition and scalar multiplication 
coordinatewise on $\H$.
We also have
\begin{align*}
...
Cauchy sequence in $\H$, where for each $n$, $x^{n} = (x^n_i)_{i\in I}$. Then given any positive 
...
For any finite subset $J\subset I$, it follows from \ref{sum1} that 
\[
...
Then $(x_i)$ is in $\H$ and by \ref{sum2}, $x^n$ converges to $(x_i)$ as $n$ tends 
...
Then $T:\H\to\H:x\mapsto Tx$ is a bounded linear operator, called the \emph{directsum} of the 
...
\begin{defn}
	...
	A \emph{$C^\ast$-algebra} is a Banach algebra $(A, \| \cdot \|)$ with involution 
...
Given a subset $S$ of a $C^\ast$-algebra $A$, let $C^\ast (S)$ denote the \emph{$C^\ast$-subalgebra
of $A$ generated by $S$}, which is the smallest $S^\ast$-subalgebra of $A$ containing $S$.
...
\begin{proof}
	...
	\end{align*}
	Then $\tilde{A}$ is a $\ast$-algebra. The norm $\|(a,\lambda)\|$ is... \todo{justify norm} %%
...
state without proof results? need spectral radius and stuff
...
\begin{itemize}
	...
	\item \emph{positive} if it is self-adjoint and $\spec(a)$ $\subseteq \R^+$.
\end{itemize}
...
$a$, respectively.
Denote the subset of positive elements in $A_{sa}$ by $A^+$. Then $A_{sa}$ is what's called a
\emph{partially ordered vector space}, with \emph{positive cone} $A^+$. 
That is to say, all $a,b \in A^+$ satisfy
\begin{enumerate}
	\item 	$a,-a\in A^+$ implies $a=0$,
	\item	$\alpha a+b\in A^+$ for all $\alpha\in \R^+$.
\end{enumerate}
The unit $\1$ is positive, and for any $a\in A_{sa}$ we have $-\|a\|\1 \leq a \leq \|a\|\1$.
\todo{justify (4.2.3(ii))} With commuting elements $a,b\in A_{sa}$, we have $(ab)^\ast = b^\ast a^\ast = ba =
ab$, so $ab$ is self-adjoint. Since $a,b,ab$ have the same spectrum in $A$ as in the Abelian 
$C^\ast$-subalgebra $C^\ast(\1,a,b)$, by our spectral theory we have 
\todo{state in 'spectrum' - 4.1.5}
\begin{align*}
...
\todo[inline]{geometric interpretation - for $A=\C$, self adjoint elements are real numbers and the
 positive cone is $\R^+$. explain $\geq$ notation } %%
...
\begin{proof} \todo{rework; explain further.}
	...
	\todo{why does this exist?} such that $\alpha\rho(a)\geq)$, and let $h$ be the real part of $a$.
...
\begin{lemma}[{\cite[4.3.4,(i)]{kadison83}}]\label{state1}
...
\begin{proof}
	...
	\end{align*}
	and as $h,k$ are self-adjoint, we must have $\rho(h)=0=\rho(k)$. By previous statement, $h=0=k$, 
...
state on $A_{sa}$.
Every pure state on $A_{sa}$ extends to a pure state on $A$, by the Hahn-Banach theorem.
We will need the following few results on pure states later.

\begin{lemma}[{\cite[4.3.8,(i)]{kadison83}}]\label{pure1}
	Let $A$ be a $C^\ast$-algebra. If $\rho(a)=0$ for all \emph{pure} states $\rho$ on $A$, then $a=0$.
\end{lemma}
\begin{proof}
	Since every state is the limit of a sequence of linear combinations of pure states, if $\rho(a)=0$ for all pure states $\rho$ on $A$, then $\rho(a)=0$ for \emph{all} states $\rho$ on $A$. The result follows immediately from Lemma \ref{state1}.
\end{proof}

\begin{prop}[{\cite[3.4.6]{kadison83}}]\label{pure2}
...
\begin{prop}\label{puremult}
	The set of pure states on an Abelian $C^\ast$-algebra $A$ is precisely the set 
	\[
		\{\rho:A \to \C ~|~ \rho(ab) =\rho(a)\rho(b)\mbox{ for all } a,b\in A\}
	\]
...
\begin{proof}[Proof.(Adapted from K\&R, 4.4.1)]
	...
	$\rho_b|_{A_{sa}} \leq \rho|_{A_{sa}}$, and it follows from Proposition \ref{pure2} that 
...
\todo[inline]{to include all representation theory, including  CGN, GNS and GN}%%
\section{Abelian $C^\ast$-algebras}
Let $A$ be an Abelian $C^\ast$-algebra. For $a$ in $A$, define a complex-valued function $\hat{a}$ on
$\P{A}$ by $\hat{a}(\rho):= \rho(a)$. The \emph{weak$^\ast$-topology} is the coarsest topology
\todo{why should this exist?} on $\P{A}$ for which
all of the maps $\hat{a}$ are continuous, so that $\hat{a} \in C(\P{A})$ for all $a\in A$. The map
\[
	\Gamma:A \to C(\P{A}) : a\mapsto \hat{a}
\]
is called the \emph{Gelfand representation} of $A$. 
...
\end{align*}
Since by Proposition \ref{puremult},
\begin{align*}
...
\end{align*}
Hence the Gelfand representation is a $\ast$-homomorphism. The following theorem gives us that it is
...
\begin{thm}[Gelfand-Naimark, commutative] 
	...
	the algebra of continuous functions on a compact Hausdorff space $X$.
\end{thm}
\begin{proof}
\end{proof}
...
\begin{thm*}
	Every commutative $C^\ast$-algebra $A$ is $\ast$-isomorphic to $C_0(X)$, the algebra of 
	continuous functions on a locally compact Hausdorff space $X$ which vanish at infinity.
\end{thm*}
The proof -- not dissimilar from the proof above -- can be found in [reference]. The unitization of
this algebra corresponds to the one-point compactificaton of $X$.
...
\end{defn}	
The following theorem is proved using the Gelfand-Naimark-Segal (or GNS) construction.
\begin{thm}
...
\begin{proof}
	...
	we conclude that $L_\rho$ is a left ideal in $A$. \todo{why do we need closure?}
	Closure  of $L_\rho$ follows from the fact that it is the preimage in $A$ of $\{0\}$ under 
	the continuous map $t \mapsto \rho (t^\ast t)$. 
...
\begin{proof}
	...
	For $a$ in $A$, we have $\|\phi_i(a)\|\leq \|a\|$ (as each $\phi_i$ is a $\ast$-homomorphism),
	so we have a bounded operator $\oplus\phi_i(a)$ on $\oplus \H_i$ by section \ref{hscons}. By the
	properties of $\oplus\phi_i(a)$ stated therein, the map 
	\[
	...
	$\rho(a)=0$, and then by Lemma \ref{pure1}, $a=0$. Thus, $\phi$ is one-to-one and hence a faithful
	representation of $A$.
\end{proof}
