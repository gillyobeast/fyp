...
\todo[inline]{move 'background mathematics' to 'definitions and results'}
...
\todo[inline]{rewrite as a summary of project chapter by chapter.} %%
...
\begin{itemize}
	...
	\item for all $x,y$ and $z$ in $X$, if $x \leq y$ and $y\leq z$, then $x\leq z$. 
												\hfill (transitivity)
\end{itemize}
\todo[inline]{define partial ordered vector space, give example of $\R$}
...
\todo[inline]{define: hausdorff/compact/locally compact/denseness/uniform continuity}
...
\todo[inline]{there is at most one $c^\ast$ norm on a $\ast$-algebra}
...
that [introduce continuous functional calculus] \todo{continuous functional calculus.}
...
\begin{align*}
	...
	r(ab) \leq r(a)r(b).
\end{align*}
\todo{this is 3.2.10/3.3.4. used in 4.1.1}
...
This result shows that $\ast$-homomorphisms between $C^\ast$-algebras are automatically continuous, 
showing that the algebraic an topological structure of these spaces are tied together by the $C^\ast$ 
axiom.
...
Some of the concepts here may be very familiar to the reader as extensions of the theory of complex numbers -- $\C$ is in fact a $C^\ast$-algebra itself. We know from complex analysis that Cauchy sequences converge in $\C$ and 
...
\begin{prop}[{\cite[4.3.2]{kadison83}}]
	...
	and $\|\rho\| = \rho(\1)$.
\end{prop}
\begin{proof} \todo[inline]{rework; explain further.}%%
...
and show \todo[inline]{say what i end up doing here}
...
\todo [inline]{define compact operators and discuss Calkin algebra?}%%
...
\todo[inline]{probably just bin this, it's a load of crap $:^)$}
