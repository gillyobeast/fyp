...
\todo[inline]{notation: $\R^+$ non neg reals. define here:  hausdorff/compact/locally compact/extreme point/denseness/weak*-topology. partially ordered vector space?
results: banach-alaoglu} %%
...
For any finite subset $J\subset I$, it follows from \ref{eqn:sum1} that 
\[
...
Then $(x_i)$ is in $\H$ and by \ref{eqn:sum2}, $x^n$ converges to $(x_i)$ as $n$ tends 
...
Recall that an \emph{extreme point} $x$ of a convex subset $X_0$ of a topological space is one for 
which an expression
\begin{align*}
	x = \alpha x_1 +(1-\alpha)x_2,
\end{align*}
for $0\leq\alpha\leq1$ and $x_1,x_2\in X_0$, implies that $x_1=x=x_2$ -- for example, the vertices of a 
polygon embedded in $\R^2$ are extreme points of that polygon.

\begin{defn*}[{\cite{rudin91}}]
Let $X$ be a topological vector space (over $\mathbb K$), with dual space $X^\ast$. Every $x$ in $X$ 
induces a linear functional $f_x$ on $X^\ast$ defined by $f_x(\rho)=\rho(x)$. The \emph{weak$^\ast$ 
topology} on $X^\ast$ is the topology generated by the sets 
\begin{align*}
	\{f^{-1}_x(V) ~|~ x \in X, V\subseteq \mathbb K \mbox{ open}\}.
\end{align*}
This is the weakest topology on $X$ such that each functional $f_x$ is continuous. 
\end{defn*}
...
\begin{defn}
	...
	A \emph{$C^\ast$-algebra} is a Banach algebra $A$ with involution map $a \mapsto a^\ast$ making it a 
...
\todo[inline]{cauchy-schwarz, quotient algebra}%%%
...
\begin{proof}
	...
	Consider $(a,\lambda)$ as an operator acting on $A$ by $b\mapsto ab+\lambda b$ for $b$ in $A$. It is 
	easy to see that this operator is linear, and 
	\begin{align*}
...
\]
Say that $\lambda\in\spec a$ is a \emph{spectral value} of $a$ in $A$. 

\begin{thm*}[{\cite[3.2.3]{kadison83}}]
	If $a$ is an element of a $C^\ast$-algebra $A$ then $\spec a$ is a non-empty closed subset of the
...
\todo[inline]{spectrum of an element is the same in a subalgebra containing it}%%



With commuting elements $a,b\in A_{sa}$, we have $(ab)^\ast = b^\ast a^\ast = ba =
...
\begin{prop}[{\cite[4.1.8]{kadison83}}]
...
\begin{proof} \todo{rework; explain further.}%%
	...
	such that $\alpha\rho(a)\geq)$, and let $h$ be the real part of $a$.
...
\emph{maximal ideal space} of $A$, denoted $\M_A$. \cite{davidson96}
...
$\P{A}$ by $\hat{a}(\rho)= \rho(a)$. The weak$^\ast$-topology is the coarsest topology on 
...
\begin{proof}
	...
	\begin{enumerate}
	  ...
	  \item Positive semi-definite: for $a \in A$:
	  	...
	  	\end{align*}
	  	since $a^\ast a$ is positive for all $a$. \todo{show: 4.2.6}%%
	\end{enumerate}
...
\begin{proof}
	...
	so we have a bounded operator $\oplus\phi_i(a)$ on $\oplus \H_i$ by section \ref{section:hscons}. By 
	the properties of $\oplus\phi_i(a)$ stated therein, the map 
	\[
