...
We begin this chapter with some definitions and an example, then give some results we will need later and finish 
...
\begin{enumerate}
	...
	\item	$f+g \in A^+$.
\end{enumerate}
The unit $\1$ is positive, and for any $a\in A_{sa}$ we have $-\|a\|\1 \leq a \leq \|a\|\1$. \todo{justify.}
...
\end{defn}\todo{show that $\|\rho\| = \rho(\1)$.} %%
Recall that an \emph{extreme} point $x$ of a convex subset $X$ of a topological space is one for which 
an expression
\begin{align*}
	x = \alpha x_1 +(1-\alpha)x_2,
\end{align*}
for $0\leq\alpha\leq1$ and $x_1,x_2\in X$, implies that $x_1=x=x_2$ -- for example, the vertices of a polygon 
embedded in $\R^2$ are extreme points of that polygon.

\todo[inline]{geometric interpretation - for $A=\C$, self adjoint elements are real numbers and the positive cone is $\R^+$} %%
...
Let $A$ be an Abelian $C^\ast$-algebra. For $a$ in $A$, define a complex-valued function $\hat{a}$ on $\P{A}$
by $\hat{a}(\rho):= \rho(a)$. The \emph{weak$^\ast$-topology} is the coarsest topology \todo{why should this exist?} on $\P{A}$ for which
all of the maps $\hat{a}$ are continuous, so that $\hat{a} \in C(\P{A})$ for all $a\in A$. The map
\[
	\Gamma:A \to C(\P{A}) : a\mapsto \hat{a}
\]
is called the \emph{Gelfand representation} of $A$. For $a,b \in A$, $\alpha,\beta\in\C$ and $\rho\in\P{A}$:
\begin{align*}
...
\end{align*}
Hence the Gelfand representation is a $\ast$-homomorphism. The following theorem gives us that it is in fact
an isometric $\ast$-isomorphism.
