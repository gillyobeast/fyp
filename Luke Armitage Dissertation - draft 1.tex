...
(real) vector space}, with \emph{positive cone} $A^+$. That is to say, all $a,b \in A^+$ satisfy
\begin{enumerate}
...
\begin{proof}
	...
	we conclude that $L_\rho$ is a left ideal in $A$. \todo{who do we need closure?}
...
Suppose we have a $C^\ast$-algebra $A$ and a collection $\{\H_i\}_{i\in I}$ of Hilbert spaces for each of which 
we have a representation $\pi_i$ of $A$ on $\H_i$. Let $K$ denote the direct sum of the collection of 
Hilbert spaces, which is a Hilbert space with pointwise addition and scalar multiplication and
norm $\|\{x_n\}\| = \big(\sum{\|x_n\|^2}\big)^{1/2}$ 
