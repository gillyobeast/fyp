...
We finish that chapter with another example, with a remark on what makes the abstract 
study of $C^\ast$-algebras applicable to quantum mechanics. 
...
\emph{A von Neumann algebra }is a $C^\ast$-subalgebra of $\BH$ which is unital and closed in the \emph{weak operator topology}, the weakest topology such that the map $T\mapsto\langle Tx,y\rangle$ is continuous for all $x,y$ in $\H$. This enforces an even stricter structure on von Neumann algebras, giving results such as the von Neumann Double Commutant theorem.
...
\begin{lemma}[{\cite[4.3.8,(iv)]{kadison83}}]\label{lemma:state2}
...
\end{defn}
The state space $\S(A)$ is then the closure, relative to the weak$^\ast$ topology, of the set of convex 
linear combinations of pure states. 
...
In this section, we show that corresponding to every state of a $C^\ast$-algebra, there is a $\ast$-homomorphism into the operator algebra $\B (\H)$ on some Hilbert space $\H$. 
\begin{defn}
